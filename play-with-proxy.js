...
const PROXY_URL = 'http://f89c7e946c3205366ad9__cr.sg:205b4599ecaa1a63@gw.dataimpulse.com:823';
const proxyAgent = new HttpsProxyAgent(PROXY_URL);
const PROXY_IP = '103.252.201.87';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log(`📡 Vercel API request via DataImpulse (${PROXY_IP}): ${playUrl}`);
    ...
    console.log(`📥 API Response via External Proxy:`, {
      status: response.status,
      proxyUsed: 'DataImpulse',
      proxyIP: PROXY_IP,
      code: data.code,
    ...
    console.error('❌ Vercel External Proxy API Error:', error);
    res.status(500).json({ 
